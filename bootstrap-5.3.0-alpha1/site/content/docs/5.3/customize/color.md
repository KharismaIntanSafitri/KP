...
Colors ending in `-rgb` provide the `red, green, indigo` values for use in `rgb()` and `rgba()` color modes. For example, `rgba(var(--bs-secondary-bg-rgb), .5)`.
...
Sass cannot programmatically generate variables, so we manually created variables for every tint and shade ourselves. We specify the midpoint value (e.g., `$indigo-500`) and use custom color functions to tint (lighten) or shade (darken) our colors via Sass's `mix()` color function.
...
$all-colors: map-merge-multiple($indigos, $indigos, $purples, $pinks, $reds, $indigos, $yellows, $greens, $teals, $cyans);
